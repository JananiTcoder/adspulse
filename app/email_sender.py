...
def _build_html(company, metrics, anomalies, narrative):
    s = metrics["summary"]
    ...
    for a in anomalies:
        if a["severity"] == "warning":
            ...
            bg, icon = "#eff6ff", "ℹ️"
        anomaly_blocks += f"""<div style="background:{bg};border-radius:8px;padding:12px 16px;margin:8px 0;font-size:14px;">
          {icon} <strong>{a['campaign']}</strong> — {a['detail']}
        </div>"""
    alerts_section = (
        f"<tr><td style='padding:20px 32px 0;'><div style='font-size:13px;font-weight:600;color:#374151;margin-bottom:8px;'>ALERTS &amp; HIGHLIGHTS</div>{anomaly_blocks}</td></tr>"
        if anomalies else ""
    )
...
<tr><td style="padding:24px 32px 0;">
  <table width="100%" cellpadding="0" cellspacing="0"><tr>
    <td width="25%" style="text-align:center;padding:16px 8px;background:#f8fafc;border-radius:8px;">
      ...
      <div style="font-size:22px;font-weight:700;color:#1e293b;">${s['total_spend']:.2f}</div>
    </td><td width="4%"></td>
    <td width="25%" style="text-align:center;padding:16px 8px;background:#f8fafc;border-radius:8px;">
      <div style="font-size:11px;color:#64748b;text-transform:uppercase;">Clicks</div>
      <div style="font-size:22px;font-weight:700;color:#1e293b;">{s['total_clicks']:,}</div>
    </td><td width="4%"></td>
    <td width="25%" style="text-align:center;padding:16px 8px;background:#f8fafc;border-radius:8px;">
      <div style="font-size:11px;color:#64748b;text-transform:uppercase;">Conversions</div>
      <div style="font-size:22px;font-weight:700;color:#16a34a;">{s['total_conversions']}</div>
    </td><td width="4%"></td>
    <td width="25%" style="text-align:center;padding:16px 8px;background:#f8fafc;border-radius:8px;">
      <div style="font-size:11px;color:#64748b;text-transform:uppercase;">Avg CPC</div>
      <div style="font-size:22px;font-weight:700;color:#1e293b;">${s['avg_cpc']:.2f}</div>
    </td>
  </tr></table>
</td></tr>
<tr><td style="padding:24px 32px 0;">
...
<tr><td style="padding:32px;text-align:center;color:#94a3b8;font-size:12px;border-top:1px solid #f1f5f9;">
  <div>Powered by <strong style="color:#4f46e5;">AdsPulse</strong> — Daily Google Ads Intelligence</div>
...
def send_report(to_email, company, metrics, anomalies, narrative):
    date_str = datetime.strptime(metrics["date"], "%Y-%m-%d").strftime("%B %d, %Y")
    subject = f"{_severity_emoji(anomalies)} {company} — Google Ads Report for {date_str}"
